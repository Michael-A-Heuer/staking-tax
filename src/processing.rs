...
pub async fn process_transactions() -> Vec<RewardEvent> {
    ...
    for tx in transactions(&client, execution_addr).await {
        ...
        if tx.to.unwrap() == execution_addr {
            event = RewardEvent::MevReward {
                reward: Reward::new(tx.time_stamp, tx.value, limiter.clone()).await,
            };
        } else {
            event = Outgoing {
                reward: Reward::new(tx.time_stamp, tx.value, limiter.clone()).await,
...
pub fn current_balance(events: &Vec<RewardEvent>) -> U256 {
    ...
    for e in events {
        match e {
            ...
            Outgoing { reward, gas } => sum -= reward.amount + gas,
        }
    }
